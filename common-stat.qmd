...
```{r}
#| label: iris-norm-data
#| tbl-cap: 
#| echo: false

kable(head(iris), caption = "iris 数据集{#tbl-kable-iris-norm}")
```


```{r}
#| layout-ncol: 2
#| label: fig-iris-data-hist
#| fig.cap: 'iris 数据集花萼长度使用不同分隔长度的直方图'
#| fig-subcap:
#|   - '0.05 的 bin 长度'
#|   - '0.3 的 bin 长度'
#|   - '0.5 的 bin 长度'
...
```{r}
#| layout-nrow: 1
#| label: fig-carqq-normqq
#| fig.cap: '两种 qqplot 的对比'
#| fig-subcap:
#|   - 'base R 的 qqplot'
#|   - 'car 的 qqplot'
library(car)
...
```{r}
#| label: fig-test-qqplot
#| fig.cap: '检验造假数据是否符合正态分布'
